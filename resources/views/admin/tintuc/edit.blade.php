        ...
        <div class="col-lg-12">
            <div class="form-group">
                ...
                <select id="danhmuc" name="danhmuc" class="form-control">
                    ...
                    <option value="{{ $list->id }}" @if($new->category_new_id == $list->id) selected @endif>{{ $list->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="tieude">Tiêu đề bài viết</label>
                <input type="text" class="form-control" id="tieude" name="tieude"  value="{{ $new->title }}">
            </div>
            <div class="form-group">
                <label for="tomtat">Tóm tắt nội dung bài viết</label>
                <input type="text" class="form-control" name="tomtat" id="tomtat"  placeholder="Tóm tắt nội dung" value="{{ $new->synopsis }}">
            </div>
            <div class="form-group">
                <label>Ảnh bài viết</label>
                <input type="file" class="form-control" name="anh_tintuc" >
                <img src="{{ $new->image ? $new->image : 'images/noimage.jpg'}}" id="anhhienthi" width="250" height="150" alt="">
            </div>
            <div class="form-group">
                <label for="noidung">Nội dung</label>
                <textarea name="noidung" id="noidung">{{ $new->content }}</textarea> 
...
<script>  
    ...
    var suatintuc = function(){
        var id = $('input[name=id_tintuc]').val();
        ...
        $.ajax({
            ...
            data:form_data
                ...
                text: data.error,
                    ...
                    window.location = 'admin/tintuc/back';
