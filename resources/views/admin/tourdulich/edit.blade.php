                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <select id="danhmuc" name="danhmuc" class="form-control">
                                    ...
                                    {{ App\Helpers\Menu::menu($lists,0,'-',$new_travel->category_id) }}
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tag">Tag</label>
                                <select id="tag_tour" name="states[]" multiple="multiple">    
                                    @foreach($tags as $tag)
                                    <option value="{{ $tag->name }}" @if(  count($new_travel->tag_tour_id->where('tag_id',$tag->id)) > 0) selected @endif>{{ $tag->name }}</option>
                                    @endforeach          
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tieude">Tiêu đề bài viết</label>
                                <input type="text" class="form-control" id="tieude" name="tieude"  value="{{ $new_travel->title }}">
                            </div>
                            <div class="form-group">
                                <label for="tomtat">Tóm tắt nội dung bài viết</label>
                                <input type="text" class="form-control" name="tomtat" id="tomtat"  placeholder="Tóm tắt nội dung" value="{{ $new_travel->synopsis }}">
                            </div>
                            <div class="form-group">
                                <label for="departure_location">Địa điểm khởi hành</label>
                                <input type="text" class="form-control" id="departure_location" name="departure_location"  placeholder="Nhập địa điểm" value="{{ $new_travel->departure_location }}">
                            </div>
                            <div class="form-group">
                                <label for="trip_time">Thời gian chuyến đi</label>
                                <input type="text" class="form-control" id="trip_time" name="trip_time"  placeholder="Nhập thời gian" value="{{ $new_travel->trip_time }}">
                            </div>
                            <div class="form-group">
                                <label for="price">Giá tour</label>
                                <input type="text" class="form-control" id="price" name="price"  placeholder="Nhập giá tour" value="{{ $new_travel->price }}" >
                            </div>
                            <div class="form-group">
                                <label for="departure_time">Ngày khởi hành</label>
                                <input type="text" class="form-control" id="departure_time" name="departure_time" value="{{ $new_travel->departure_time }}">
                            </div>
                            <div class="form-group">
                                <label class="radio">Phương tiện</label>
                                @if($new_travel->vehicle == 0)
                                <input type="radio" class="flat" value="0" name="vehicle" checked>Máy bay+Xe<br/>
                                <input type="radio" class="flat" value="1" name="vehicle">Xe
                                @else
                                <input type="radio" class="flat" value="0" name="vehicle">Máy bay+Xe<br/>
                                <input type="radio" class="flat" value="1" name="vehicle" checked>Xe
                                @endif
                            </div>
                            <div class="form-group">
                                <label>Ảnh bài viết</label>
                                <input type="file" class="form-control" name="anh_tintuc" >
                                <img src="{{ $new_travel->image ? $new_travel->image : 'images/noimage.jpg'}}" id="anhhienthi" width="250" height="150" alt="">
                            </div>
                        </div>
                    </div>
                    <div role="tabpanel" class="tab-pane fade" id="tab_content2" aria-labelledby="profile-tab">
                        <div class="form-group">
                            <textarea name="tour_schedule" id="tour_schedule">{{ $new_travel->tour_schedule }}</textarea> 
                            <script type="text/javascript">CKEDITOR.replace('tour_schedule'); </script>   
                        </div>
                    </div>
                    <div role="tabpanel" class="tab-pane fade" id="tab_content3" aria-labelledby="profile-tab">
                        <div class="form-group">
                            <textarea name="regulations" id="regulations">{{ $new_travel->regulations }}</textarea> 
                            <script type="text/javascript">CKEDITOR.replace('regulations'); </script>   
                        </div>
                    </div>
                    <div role="tabpanel" class="tab-pane fade" id="tab_content4" aria-labelledby="profile-tab">
                        <div class="form-group">
                            <textarea name="rules" id="rules">{{ $new_travel->rules }}</textarea> 
                            <script type="text/javascript">CKEDITOR.replace('rules'); </script>   
                        </div>
                    </div>
                </div>
            </div>
            <button class="btn btn-primary pull-right" onclick="suatintuc();">Save</button>
    ...
    var suatintuc = function(){
        ...
        var error = '';
            ...
            $.ajax({
                ...
                type:'post',
                url:'{{ url("admin/tourdulich/edit") }} +'/' + id',
                contentType: false,
                processData: false,
                data:form_data
                }).done(function(rs){
